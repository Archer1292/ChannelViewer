...
Begin
    Case wErrcode of
	 0: ErrString := 'Без ошибок';//'No Error' ;
	 1: ErrString := 'Function Error';
	 2: ErrString := 'Ошибка порта';//'Port Error';
	 ...
	 8: ErrString :='Com-порт не открыт'; //'ComPort Not Open';
	...
	//14: ErrString := '';
	15: ErrString := 'Модуль не отвечает слишокм долго';//'Time Out';
	//16: ErrString := '';
    ...
    Else
	    ErrString := 'Неизвестная ошибка. Возможно модуль не найден.';
    end;
