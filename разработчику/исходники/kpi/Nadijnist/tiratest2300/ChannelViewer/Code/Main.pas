...
begin
  DataThread.DataCollectingThread.FreeOnTerminate := False;
  DataThread.DataCollectingThread.Free;
  Graph.Free;
end;
...
begin
    ...
    lblDataEntities.Caption  := IntToStr(Pos);
    //Graph.Continue;
end;
...
begin
  try
     ...
     Box.Left := StrToFloat(StringReplace(edLeft.Text, ',', '.',[rfReplaceAll, rfIgnoreCase]));
     Box.Right := StrToFloat(StringReplace(edRight.Text, ',', '.',[rfReplaceAll, rfIgnoreCase]));
     Box.Up := StrToFloat(StringReplace(edUp.Text, ',', '.',[rfReplaceAll, rfIgnoreCase]));
     Box.Down := StrToFloat(StringReplace(edDown.Text, ',', '.',[rfReplaceAll, rfIgnoreCase]));
  except
...
begin
    ...
    if (DataTimer.Enabled) then
        Button2.Caption := 'Stop recird'
    else
        Button2.Caption := 'Write in table';
end;
...
procedure TFormMain.SaveToExcel(Sender: TObject);  { Todo созранять коректоно}
const
...
// Создаём оформление документа
    ...
    s:= AnsiToUTF8('Report ') + DateToStr(Date) +AnsiToUTF8(' Time: ') + TimeToStr(Time);
    Sheet.Cells[1, 4] :=s;
...
// Уничтожаем демонстрационную форму
   ...
   //   Excel.DisplayAlerts := False;
    ...
    try
       ...
       Excel.Workbooks[1].saveas(dlgExcel.FileName, xlExcel9795);
      // showmessage('???? ???????? 2003-?? ????');
    except
      //?????? xls 97-2003 ???? ?????????? 2007-2010 Excel
      Excel.Workbooks[1].saveas(dlgExcel.FileName, xlExcel8);
      //showmessage('???? ???????? 2007 ??? 2010-?? ????');
    end;

    Excel.Quit;
    VarClear(Excel);  // Excel := Unassigned;
...
begin
   if DataThread.DataCollectingThread.Suspended then
   DataThread.DataCollectingThread.Resume;
   if DataThread.DataCollectingThread.ConnectToModul then DataThread.DataCollectingThread.PrepareModule;
end;
